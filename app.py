...
class Order:    #класс заявок
    ...
    def __init__(self, id, shop_id, date=[], shop='', target='Пустая', status_out='new', status_in='new', photo = [], estimate = [], material = [], works = [], comments = [], color = ''):
        self.id = id
    ...
    for id in bufer:
        if filter_shop and filter_shop != 'all':
            if str(filter_shop) != str(id['shop_id']):
                continue
        order_card.append(Order(id=id['id'], shop_id=id['shop_id'], shop=id['shop'], target=id['target'], status_in=id['status_in'], status_out=id['status_out'], color=id['color']))
        print(id)
...
def make_order():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                shop = dict_shop[shop_id] if shop_id in dict_shop else 'noname'
                with connection.cursor() as cursor:
                    insert = '''INSERT INTO orders (id, shop_id, shop, target) VALUES ('{}', '{}', '{}', '{}');'''.format(id,
                                                                                                                          shop_id,
                                                                                                                          shop,
                                                                                                                          target)
...
def add_shop():
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                with connection.cursor() as cursor:
                    insert = '''INSERT INTO shops (shop_id, shop_name, shop_direct, shop_engeener, shop_secure, direct_phone, engeener_phone, secure_phone) VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}');'''.format(shop_id, shop_name, shop_direct, shop_engeener, shop_secure, direct_phone, engeener_phone, secure_phone)
                    cursor.execute(insert)
                    connection.commit()
...
def edit_shop(shop_id):
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                with connection.cursor() as cursor:
                    update = '''UPDATE shops SET shop_name=%s, shop_direct=%s, shop_engeener=%s, shop_secure=%s, direct_phone=%s, engeener_phone=%s, secure_phone=%s WHERE shop_id = %s'''
                    data = [shop_name, shop_direct, shop_engeener, shop_secure, direct_phone, engeener_phone, secure_phone, shop_id]
                    cursor.execute(update, data)
                    connection.commit()
